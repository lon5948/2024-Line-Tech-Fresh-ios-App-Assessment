//
//  PharmacyTableViewController.swift
//  2024-Line-Tech-Fresh-ios-App-Assessment
//
//  Created by LON Li on 2024/4/18.
//

import UIKit

class PharmacyTableViewController: UITableViewController {

    var pharmacies = [Pharmacy]()
    var refreshCon: UIRefreshControl!
    
    override func viewDidLoad() {
        super.viewDidLoad()
        self.title = "健保特約醫事機構-藥局"
        refreshCon = UIRefreshControl()
        refreshCon.addTarget(self, action: #selector(refresh), for: .valueChanged)
        tableView.addSubview(refreshCon)
        
        let urlStr = "https://info.nhi.gov.tw/api/iode0010/v1/rest/datastore/A21030000I-D21005-001"
        
        if let url = URL(string: urlStr) {
            URLSession.shared.dataTask(with: url) { (data, response, erroe) in
                let decoder = JSONDecoder()
                if let data = data {
                    do {
                        let responseData = try decoder.decode(ResponseData.self, from: data)
                        DispatchQueue.main.async {
                            self.pharmacies = responseData.result.records.filter { $0.close.isEmpty }
                            self.tableView.reloadData()
                        }
                    } catch {
                        print(error)
                    }
                }
            }.resume()
        }
    }

    // MARK: - Table view data source

    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return pharmacies.count
    }

    
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "pharmacyCell", for: indexPath) as! PharmacyTableViewCell
        
        let pharmacy = pharmacies[indexPath.row]
        cell.titleLabel.text = pharmacy.name
        
        return cell
    }
    
    @IBSegueAction func ToPharmacyDetailPage(_ coder: NSCoder) -> PharmacyDetailViewController? {
        let controller =  PharmacyDetailViewController(coder: coder)
        if let row = tableView.indexPathForSelectedRow?.row {
            controller?.pharmacy = pharmacies[row]
        }
        return controller
    }
    
    @objc func refresh() {
        tableView.reloadData()
        refreshCon.endRefreshing()
    }
}
